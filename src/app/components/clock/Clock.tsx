...
export default function Clock({
  ...
  const timePassed =
    currentTime.getTime() - startTime.getTime() - pausedDuration
  const timeRemaining = finishTime.getTime() - currentTime.getTime()
  const timePerSubdivision = fullRoundDuration / subdivisions
  const remainingTimePerSubdivision =
    timePerSubdivision - (timePassed % timePerSubdivision)
  ...
  const subdivisionSeconds = Math.floor(
    (remainingTimePerSubdivision % 60000) / 1000
  )
      ...
      <div className="h-32 flex flex-col justify-center items-center">
        ...
        {!isFinished && (
          <div className="flex flex-col gap-3 justify-center items-center">
            ...
            <DigitalClock
              minutes={subdivisionMinutes}
              seconds={subdivisionSeconds}
              size="small"
            />
          </div>
        )}
