...
import SubdivisionSelector, {
  subdivisionOptions,
} from '@/app/components/time-control/SubdivisionSelector'
import LengthSelector, {
  lengthOptions,
} from '@/app/components/time-control/LengthSelector'

export default function TimeControl() {
  const [selectedLengthOption, setSelectedLengthOption] = useState(0)
  const [selectedSubdivisionsOption, setSelectedSubdivisionsOption] =
    useState(0)
  ...
  useEffect(() => {
    ...
    return () => {
      if (wakeLock)
        wakeLock
          .release()
          .then(() => console.log('Wake Lock has been released'))
    }
  }, [isInProgress])
  ...
  function handleStart() {
    ...
    setSubdivisionTimes(
      Array.from(
        { length: subdivisions },
        (_, i) => now + subdivisionDuration * (i + 1)
      )
    )
  }
  ...
  function playCricketSound() {
    const cricketSound = document.getElementById(
      'cricket_sound'
  ...
  function playBellSound() {
    const bellSound = document.getElementById(
      'bell_sound'
    ) as HTMLAudioElement | null
    if (bellSound) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (
        newSubdivision !== currentSubdivision &&
        newSubdivision >= 0 &&
        newSubdivision < subdivisions
      ) {
      ...
      {!isInProgress && (
        <>
          ...
          <SubdivisionSelector
            ...
            setSelectedSubdivisionsOption={setSelectedSubdivisionsOption}
            selectedLengthOption={selectedLengthOption}
          />
